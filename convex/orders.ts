...
export const createOrder = mutation({
  args: {
    ...
    deliveryAddress: v.optional(v.object({ 
      ...
      // zoneName and deliveryFee removed, will be derived from deliveryZoneId
    })),
    deliveryZoneId: v.optional(v.id("deliveryZones")), // Added deliveryZoneId
    ...
    // Removed deliveryFee from args, will be calculated
  },
  handler: async (ctx, args) => {
    ...
    if (args.orderType === 'Delivery') {
      if (!args.deliveryZoneId) {
        throw new Error("Delivery zone ID is required for delivery orders.");
      }
    ...
    const newOrderId = await ctx.db.insert("orders", {
      ...
      ...(args.orderType === 'Delivery' && {
        ...
        deliveryZoneId: args.deliveryZoneId!, // Store the selected zone ID
        deliveryFee: deliveryFee, // Store the calculated delivery fee
      }),
    ...
    await ctx.scheduler.runAfter(0, api.miscMutations.updateUserActivity, { userId: args.userId });
    return newOrderId;
  },
});
