...
export default function CheckoutPage() {
  ...
  const handlePayment = async () => {
    ...
    try {
      const completeDeliveryAddress = requiresAddress
        ? {
      ...
      const dineInDetails = requiresDineInDetails
        ? {
      ...
      const orderPayload = {
        ...
        appliedPromoId:
          validationResult && 'promoId' in validationResult
            ? validationResult.promoId
            : undefined,
        ...(requiresAddress && { deliveryAddress: completeDeliveryAddress }),
        ...(requiresPickupTime && {
          pickupTime: Date.parse(`1970-01-01T${pickupTime}:00`),
        }),
        ...dineInDetails,
      };

      const orderId = await createOrder(orderPayload as any);
