...
import { type DeliveryZone as DeliveryZoneType } from '@/convex/_generated/dataModel'; // Import DeliveryZone type if needed elsewhere, or rely on Doc<"deliveryZones">
...
export default function CheckoutPage() {
  ...
  const finalTotalWithDelivery = finalTotal + (deliveryFee / 100);
  ...
  // Check if current time is peak hour (you can adjust the hours as needed)
  useEffect(() => {
    const now = new Date();
    const hour = now.getHours();
    // Consider 6 PM - 9 PM (18:00 - 21:00) as peak hours
  ...
  const handlePayment = async () => {
    ...
    try {
      const completeDeliveryAddress = requiresAddress && deliveryAddress
        ? {
          street: deliveryAddress.street ?? '', // Ensure string
          customerPhone: deliveryAddress.customerPhone ?? '', // Ensure string
          ...
          recipientPhone: deliveryAddress.isOrderingForSomeoneElse
            ...
            : undefined,
          zoneName: selectedZone ?? undefined, // Convert null to undefined
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <Card>
            ...
            <div className="space-y-4">
              ...
              {selectedOrderType === 'Delivery' && (
                <div className="space-y-4">
                  ...
                  <Separator />
                  {/* Removed conditional rendering based on zoneData */}
                  <DeliveryZoneSelector
                    // deliveryZones={zoneData} // Removed prop
                    selectedZoneId={selectedZoneId} // Pass selectedZoneId
                    isPeakHour={isPeakHour}
                    onZoneSelect={(zone) => { // Receives full zone object
                      setSelectedZoneId(zone._id); // Set the ID
                      setDeliveryFee(isPeakHour ? zone.peakFee : zone.baseFee);
                    }}
                  />
                  <AddressForm
                    onSubmit={(address) => {
                      if (!selectedZoneId) { // Check selectedZoneId
                        toast({
                          title: "Select Delivery Zone",
                          description: "Please select your delivery zone first.",
                          variant: "destructive"
                        });
                        return;
                      }
                      setDeliveryAddress({
                        ...address,
                        // zoneName: selectedZone, // Maybe remove if not needed, or get from selected zone object if required elsewhere
                      });
                      toast({ title: "Address Saved", description: "Delivery address updated." });
                    }}
          ...
          <CardFooter className="flex justify-between">
            ...
            >
              ...
              {isSubmitting
                ? 'Processing...'
                : `Proceed to Payment ($${finalTotalWithDelivery.toFixed(2)})`}
            </Button>
          </CardFooter>
        </Card>
      </div>
      ...
      if (selectedOrderType === 'Delivery') {
        if (!selectedZoneId || !deliveryAddress || !deliveryFee) {
          toast({ title: "Missing Delivery Information", description: "Please select a delivery zone and provide an address.", variant: "destructive" });
          setIsSubmitting(false);
          return;
        }
        orderData.deliveryZoneId = selectedZoneId;
