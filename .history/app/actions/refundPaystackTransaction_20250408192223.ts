...
export async function refundPaystackTransaction(orderId: Id<"orders">) {
  ...
  let refundResult: { success: boolean; message?: string; refundData?: any } = { success: false };
  ...
  try {
     ...
     let responseData: PaystackRefundResponse | { message?: string, status?: boolean }; 
     try {
     ...
     const actualRefundData = (responseData as PaystackRefundResponse).data.transaction; 
