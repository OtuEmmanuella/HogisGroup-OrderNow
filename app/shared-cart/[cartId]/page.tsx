...
import DeliveryZoneSelectorModal from '@/components/DeliveryZoneSelectorModal';
import type { DeliverySelectionData } from '@/components/DeliveryZoneSelectorModal'; // Import type separately
    ...
    <div className="container mx-auto px-4 py-8 max-w-4xl">
      ...
      <Card className="mb-6">
        ...
        <CardContent className="space-y-3">
          <div className="flex justify-between items-center">
            <span>Status:</span> 
            <Badge variant={status === 'completed' ? 'default' : status === 'open' ? 'secondary' : status === 'cancelled' ? 'destructive' : 'default'}>
              {status}
            </Badge>
          </div>
          <div className="flex justify-between items-center">
      ...
      <Card className="mt-6">
         ...
         <CardContent className="space-y-4">
             ...
             {/* Pay Buttons (Allow paying when status is 'open' or 'paying', disable if already paid) */} 
             {(status === 'open' || status === 'paying') && paymentMode === 'split' && (
                <Button
                 className="w-full"
                 onClick={handlePayMyShare}
                 disabled={isPayingMyShare || !canPay || currentUserMemberInfo?.paymentStatus === 'paid'}
                 size="lg"
                >
                  {isPayingMyShare 
                    ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Processing...</> 
                    : currentUserMemberInfo?.paymentStatus === 'paid' 
                      ? 'Your Share is Paid' 
